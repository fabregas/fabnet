...
class TestAbstractFriServer(unittest.TestCase):
    def __start_server(self, processor_class, routine, keystorage=None):
        server_name = 'test-node'
        cur_thread = threading.current_thread()
        cur_thread.setName('%s-main'%server_name)
        workers_mgr = WorkersManager(processor_class, min_count=1, max_count=8, server_name=server_name)
    ...
    def test01_workers_nossl_spawn_stop(self):
        def stress_routine():
            def call_method():
                fri_client = FriClient()
